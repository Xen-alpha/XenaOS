...
kSwitchAndExecute64bitKernel:
    ...
    or eax, 0x620    ; PAE 비트(비트 5), OSXMMEXCPT 비트, OSFXSR 비트를 1로 설정
    mov cr4, eax    ; PAE 비트가 1로 설정된 값을 CR4 컨트롤 레지스터에 저장
    
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; CR3 컨트롤 레지스터에 PML4 테이블의 어드레스 및 캐시 활성화
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    mov eax, 0x400000   ; EAX 레지스터에 PML4 테이블이 존재하는 0x100000(1MB)를 저장
    ...
    or eax, 0x0101      ; EAX 레지스터에 저장된 IA32_EFER MSR의 하위 32비트에서 
                        ; LME 비트(비트 8)을 1로 설정 + SCE bit를 1로 설정
    wrmsr               ; MSR 레지스터에 쓰기
    ...
    or eax, 0xE000000E      ; NW 비트(비트 29), CD 비트(비트 30), PG 비트(비트 31), TS비트, EM비트, MP비트를 모두 1로 설정
    xor eax, 0x60000004     ; NW 비트(비트 29)와 CD 비트(비트 30), EM, MP을 XOR하여 0으로 설정
    mov cr0, eax            ; NW 비트 = 0, CD 비트 = 0, PG 비트 = 1로 설정한 값을 다시 
                            ; CR0 컨트롤 레지스터에 저장
    
    jmp 0x08:0x200000   ; CS 세그먼트 셀렉터를 IA-32e 모드용 코드 세그먼트 디스크립터로
                        ; 교체하고 0x200000(4MB) 어드레스로 이동
