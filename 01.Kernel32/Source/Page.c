...
#define DYNAMICMEMORY_START_ADDRESS     ((TASK_STACKPOOLADDRESS +0x1fffff) & 0xffe00000) // 64비트 커널의 DynamicMemory.h와 동기화되어야 함
/**
...
void kInitializePageTables( void )
{
	...
	pstPML4TEntry = ( PML4TENTRY* ) 0x400000;
	// 페이지 엔트리를 유저 레벨로 설정하여 유저 레벨에서 접근 가능하도록 설정
    kSetPageEntryData( &( pstPML4TEntry[ 0 ] ), 0x00, 0x401000, PAGE_FLAGS_DEFAULT | PAGE_FLAGS_US, 0 );
	...
	pstPDPTEntry = ( PDPTENTRY* ) 0x401000;
	for( i = 0 ; i < 64 ; i++ )
	{
		kSetPageEntryData( &( pstPDPTEntry[ i ] ), 0, 0x402000 + ( i * PAGE_TABLESIZE ), 
