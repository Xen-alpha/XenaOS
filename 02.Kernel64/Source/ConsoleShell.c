...
static void kTestSemaphore( const char* pcParameterBuffer ) {
    ...
    for (i = 0 ; i < 3; i++) {
        kCreateTask(TASK_FLAGS_LOW | TASK_FLAGS_THREAD, 0, 0, (QWORD) kPrintSemaphoreTask);
    }
    kPrintf( "Wait Util %d Task End...\n", i );
    kGetCh();
}
...
static void kShowRootDirectory( const char* pcParameterBuffer )
{
    ...
    while( 1 )
    {
        ...
        memcpy( vcBuffer + 30, vcTempValue, kStrLen( vcTempValue ) );

        // 파일의 시작 클러스터 삽입
        kSPrintf( vcTempValue, "0x%X Cluster", pstEntry->dwStartClusterIndex );
        memcpy( vcBuffer + 55, vcTempValue, kStrLen( vcTempValue ) + 1 );
        kPrintf( "    %s\n", vcBuffer );
...
static void kTestFileIO( const char* pcParameterBuffer )
{
    ...
    for( i = 0 ; i < 100 ; i++ )
    {
        ...
        // 버퍼와 비교
        if( memcmp( pbBuffer + dwRandomOffset, vbTempBuffer, dwByteCount ) 
                != 0 )
        {
            kPrintf( "[Fail]\n" );
