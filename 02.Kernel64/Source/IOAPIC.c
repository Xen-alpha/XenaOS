...
QWORD kGetIOAPICBaseAddressOfPCI( void )
{
    ...
    // I/O APIC의 어드레스가 저장되어 있지 않으면 엔트리를 찾아서 저장
    if( gs_stIOAPICManager.qwIOAPICBaseAddressOfPCI== NULL )
    {
        pstIOAPICEntry = kFindIOAPICEntryForPCI();
        if( pstIOAPICEntry != NULL )
        {
            gs_stIOAPICManager.qwIOAPICBaseAddressOfPCI = 
                pstIOAPICEntry->dwMemoryMapAddress & 0xFFFFFFFF;
        }
    }

    // I/O APIC의 기준 어드레스를 찾아서 저장한 다음 반환
    return gs_stIOAPICManager.qwIOAPICBaseAddressOfPCI;
}
...
void kReadIOAPICRedirectionTable( int bus, int iINTIN, IOREDIRECTIONTABLE* pstEntry )
{
    ...
    if (bus == 0){
        ...
        // PCI 버스가 연결된 I/O APIC의 메모리 맵 I/O 어드레스
        qwIOAPICBaseAddress = kGetIOAPICBaseAddressOfPCI();
    }
...
void kMaskAllInterruptInIOAPIC( int bus )
{
    ...
    for( i = 0 ; i < IOAPIC_MAXIOREDIRECTIONTABLECOUNT ; i++ )
    {
        ...
        // 설정하여 다시 저장
        kReadIOAPICRedirectionTable( bus,i, &stEntry );
        stEntry.bInterruptMask = IOAPIC_INTERRUPT_MASK;
        kWriteIOAPICRedirectionTable( bus ,i, &stEntry );
    }
}
...
void kInitializeIORedirectionTable( void )
{
    ...
    kMaskAllInterruptInIOAPIC(0);
    kMaskAllInterruptInIOAPIC(1);
    
    // IO 인터럽트 지정 엔트리 중에서 ISA 및 PCI 버스와 관련된 인터럽트만 추려서 I/O 리다이렉션
    // 테이블에 설정
    ...
    for( i = 0 ; i < pstMPHeader->wEntryCount ; i++ )
    {
        ...
        case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
                ...
                ( pstIOAssignmentEntry->bInterruptType == MP_INTERRUPTTYPE_INT ) )                        
            {
                ...
                // 이용하여 처리
                kWriteIOAPICRedirectionTable( 0, pstIOAssignmentEntry->bDestinationIOAPICINTIN, 
            ...
            ( pstIOAssignmentEntry->bInterruptType == MP_INTERRUPTTYPE_INT )) {
                ...
                // 이용하여 처리
                kWriteIOAPICRedirectionTable( 1,pstIOAssignmentEntry->bDestinationIOAPICINTIN, 
                        &stIORedirectionEntry );
...
void kRoutingIRQToAPICID( int iIRQ, BYTE bAPICID )
{
    ...
    QWORD qwEntryAddress = kGetMPConfigurationManager()->qwBaseEntryStartAddress;
    ...
    for( i = 0 ; i < pstMPHeader->wEntryCount ; i++ )
    {
        switch( *( BYTE* ) qwEntryAddress )
        {
        case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
            pstIOAssignmentEntry = ( IOINTERRUPTASSIGNMENTENTRY* ) qwEntryAddress;
            // IO 인터럽트 지정 엔트리이면, ISA 버스인지 확인하여 I/O 리다이렉션
            if( ( pstIOAssignmentEntry->bSourceBUSID == kGetMPConfigurationManager()->bISABusID ) )                        
            {
                // 설정된 I/O 리다이렉션 테이블을 읽어서 목적지(Destination) 필드만 수정
                kReadIOAPICRedirectionTable( 0, gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ],&stEntry );
                stEntry.bDestination = bAPICID;
                kWriteIOAPICRedirectionTable( 0, gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ], &stEntry );
            } 
            // 이번에는 PCI 버스
            else if (( pstIOAssignmentEntry->bSourceBUSID == kGetMPConfigurationManager()->bPCIBusID ) ) {
                // 설정된 I/O 리다이렉션 테이블을 읽어서 목적지(Destination) 필드만 수정
                kReadIOAPICRedirectionTable( 1, gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ],&stEntry );
                stEntry.bDestination = bAPICID;
                kWriteIOAPICRedirectionTable( 1, gs_stIOAPICManager.vbIRQToINTINMap[ iIRQ ], &stEntry );
            }
            qwEntryAddress += sizeof( IOINTERRUPTASSIGNMENTENTRY );
            break;
