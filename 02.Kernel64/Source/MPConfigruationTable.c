...
BOOL kAnalysisMPConfigurationTable( void )
{
    ...
    for( i = 0 ; i < pstMPConfigurationHeader->wEntryCount ; i++ )
    {
        ...
        case MP_ENTRYTYPE_BUS:
            ...
            } else if( memcmp( pstBusEntry->vcBusTypeString, MP_BUS_TYPESTRING_PCI,
                    kStrLen( MP_BUS_TYPESTRING_PCI ) ) == 0 )
            {
                gs_stMPConfigurationManager.bPCIBusID = pstBusEntry->bBusID;
            }
