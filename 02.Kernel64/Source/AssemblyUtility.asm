...
global kInPortByte, kOutPortByte, kInPortWord, kOutPortWord, kInPortDword, kOutPortDword
...
kInPortDword:
    ...
    mov rdx, rdi    ; RDX 레지스터에 파라미터 1(포트 번호)를 저장
    mov rax, 0      ; RAX 레지스터를 초기화
    in eax, edx       ; EDX 레지스터에 저장된 포트 어드레스에서 네 바이트를 읽어
                    ; EAX 레지스터에 저장, EAX 레지스터는 함수의 반환 값으로 사용됨
...
kOutPortDword:
    ...
    mov rdx, rdi    ; RDX 레지스터에 파라미터 1(포트 번호)를 저장
    mov rax, rsi    ; RAX 레지스터에 파라미터 2(데이터)를 저장    
    out edx, eax      ; DX 레지스터에 저장된 포트 어드레스에 AX 레지스터에 저장된
                    ; 네 바이트를 씀
